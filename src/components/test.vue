  ...
  methods: {
    mouseEvent () {
      ...
      function showMenu () {
        ...
        if (rightedge < menu.offsetWidth) { menu.style.left = documentScrollLeft + container.scrollLeft + evt.clientX - menu.offsetWidth + 'px' }
        // else if(rightedge)
        else
        /* 否则，就定位菜单的左坐标为当前鼠标位置 */ { menu.style.left = documentScrollLeft + container.scrollLeft + evt.clientX + 'px' }

        /* 如果从鼠标位置到容器下边的空间小于菜单的高度，就定位菜单的上坐标（Top）为当前鼠标位置向上一个菜单高度 */
        if (bottomedge < menu.offsetHeight) { menu.style.top = documentScrollTop + container.scrollTop + evt.clientY - menu.offsetHeight + 'px' } else
        /* 否则，就定位菜单的上坐标为当前鼠标位置 */ { menu.style.top = documentScrollTop + container.scrollTop + evt.clientY + 'px' }
        /* 设置菜单可见 */
